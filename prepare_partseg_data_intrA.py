...
def main():
    ...
    root = args.folder if args.folder else r'E:\ZXY\Data_new'
    folders = [(os.path.join(root,'test_0_p_lc'), os.path.join(root,'test_0_label'))]
    print(folders)
    ...
    for data_folder, label_folder in folders:
        ...
        for category in sorted(os.listdir(data_folder)):
            ...
            print("data_category_folder:",data_category_folder)
            ...
            for filename in sorted(os.listdir(data_category_folder)):
                ...
                if label_folder is not None:
                    ...
                    label_seg_this = np.loadtxt(label_filepath,encoding='utf-8').astype(np.int32)
    ...
    print("category_label_seg_max_list:",category_label_seg_max_list)

    category_label = dict()
    offset = 0
    category_offset = dict()
    label_seg_max = max([category_label_seg_max for _, category_label_seg_max in category_label_seg_max_list])
    with open(os.path.join(root, 'synsetoffset2category.txt'), 'w') as file_categories:
    ...
    train_val_txt = os.path.join(root, "train_files.txt")
    with open(train_val_txt, "w") as train_val:
        part_txt = os.path.join(root, "%s_files.txt" % "train")
